...
//|          الإصدار: 5.0 - Pullback to EMA Strategy                |
//+------------------------------------------------------------------+
...
input group "=== مؤشرات ==="
input int    EMA_Period      = 200;   // H4 EMA للاتجاه الرئيسي
input int    H1_EMA_Period   = 21;    // H1 EMA للـ Pullback (نقطة الارتداد)
input int    W1_EMA_Period   = 50;    // W1 EMA للاتجاه الأسبوعي
...
input double PullbackZone    = 0.0010; // مسافة الـ Pullback من الـ EMA (pip distance)
...
//+------------------------------------------------------------------+
//| GetSignal - Pullback to EMA Strategy                            |
//|                                                                  |
//| الفكرة:                                                          |
//| 1. W1 + H4 يحددان الاتجاه الرئيسي                              |
//| 2. ADX يؤكد قوة الترند                                          |
//| 3. السعر يرجع (Pullback) للـ H1 EMA21                          |
//| 4. RSI يؤكد منطقة الارتداد                                      |
//| 5. M5 Engulfing/PinBar يؤكد الارتداد                           |
//+------------------------------------------------------------------+
int GetSignal(string symbol) {
   ...
   double h4ema[];
   int h4Handle = iMA(symbol, PERIOD_H4, EMA_Period, 0, MODE_EMA, PRICE_CLOSE);
   if(h4Handle == INVALID_HANDLE) return 0;
   ArraySetAsSeries(h4ema, true);
   CopyBuffer(h4Handle, 0, 0, 3, h4ema);
   IndicatorRelease(h4Handle);
   double lastH4Close = iClose(symbol, PERIOD_H4, 1);
   bool bullH4 = lastH4Close > h4ema[0];
   bool bearH4 = lastH4Close < h4ema[0];

   // W1 و H4 لازم يتفقان
   if(bullW1 != bullH4) return 0;
   if(bearW1 != bearH4) return 0;

   // === فلتر 3: ADX > MinLevel (ترند قوي) ===
   double adxVal[];
   ...
   // === فلتر 4: H1 EMA 21 - نقطة الـ Pullback ===
   double h1ema[];
   int h1Handle = iMA(symbol, PERIOD_H1, H1_EMA_Period, 0, MODE_EMA, PRICE_CLOSE);
   if(h1Handle == INVALID_HANDLE) return 0;
   ArraySetAsSeries(h1ema, true);
   CopyBuffer(h1Handle, 0, 0, 3, h1ema);
   IndicatorRelease(h1Handle);

   double currentPrice = iClose(symbol, PERIOD_H1, 1);
   double emaDistance  = MathAbs(currentPrice - h1ema[0]);

   // السعر لازم يكون قريب من الـ EMA (Pullback Zone)
   bool nearEMA = emaDistance <= PullbackZone;
   if(!nearEMA) return 0;

   // السعر لازم يكون على الجانب الصحيح من الـ EMA
   bool priceAboveEMA = currentPrice > h1ema[0]; // BUY: السعر فوق EMA
   bool priceBelowEMA = currentPrice < h1ema[0]; // SELL: السعر تحت EMA

   // === فلتر 5: RSI - تأكيد منطقة الارتداد ===
   double rsiVal[];
   int rsiHandle = iRSI(symbol, PERIOD_H1, RSI_Period, PRICE_CLOSE);
   if(rsiHandle == INVALID_HANDLE) return 0;
   ArraySetAsSeries(rsiVal, true);
   CopyBuffer(rsiHandle, 0, 0, 3, rsiVal);
   IndicatorRelease(rsiHandle);

   // BUY:  RSI بين 35-55 (جاء من فوق ونزل للـ EMA)
   // SELL: RSI بين 45-65 (جاء من تحت وصعد للـ EMA)
   bool rsiBuy  = (rsiVal[0] >= 35.0 && rsiVal[0] <= 55.0);
   bool rsiSell = (rsiVal[0] >= 45.0 && rsiVal[0] <= 65.0);

   // === فلتر 6: MACD - اتجاه الزخم ===
   ...
   bool macdBull = macdMain[0] > macdSig[0];
   bool macdBear = macdMain[0] < macdSig[0];

   // === فلتر 7: M5 - تأكيد الارتداد (Engulfing أو Pin Bar) ===
   bool bullM5 = IsBullishCandle(symbol);
   bool bearM5 = IsBearishCandle(symbol);

   // === تجميع شروط BUY ===
   // ترند صاعد + السعر رجع للـ EMA من فوق + RSI مناسب + MACD صاعد + شمعة تأكيد
   if(bullW1 && bullH4 && priceAboveEMA && rsiBuy  && macdBull && bullM5) return 1;

   // === تجميع شروط SELL ===
   // ترند هابط + السعر رجع للـ EMA من تحت + RSI مناسب + MACD هابط + شمعة تأكيد
   if(bearW1 && bearH4 && priceBelowEMA && rsiSell && macdBear && bearM5) return -1;
